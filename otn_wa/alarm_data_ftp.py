...
alarm_ftp_trace_file = 'alarm_trace.json'
alarm_local_base_path = '/alu/DEPOT/BackupJobs'
...
def alarm_file_list():
    alarm_local_files = []
    brief = []
    for item in os.listdir(alarm_local_base_path):
        path = alarm_local_base_path # "/alu/DEPOT/BackupJobs"
        if item[0:6].lower() == 'alarm_':
            date = item
            path = path + '/' + item # "/alu/DEPOT/BackupJobs/alarm_20200415_163415"
    ...
    update_log_by_local_files(alarm_local_files)
    return brief

def get_alarm_upload_log():
    f = open(alarm_ftp_trace_file, 'r', encoding='utf-8')
...
def update_log(file_key, remote_location, remote_name, ip, port):
    ...
    f = open(alarm_ftp_trace_file, 'w', encoding='utf-8')
...
def init():
    if not os.path.exists(alarm_ftp_trace_file):
